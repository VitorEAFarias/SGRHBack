...
namespace Utilitarios.Utilitários.PDF
{
    ...
    public class EPIRelatorioPDF
    {
        ...
        /// <returns></returns>
        public static string GetHTMLString(List<EPIRelatorioProdutosDTO> dadosPDF)
        {
            ...
            foreach (var item in dadosPDF)
            {
                sb.AppendFormat(@"<tr>
                                    ...
                                    <td>{5}</td>
                                    <td>{6}</td>
                                  </tr>", item.numeroPedido, item.dataPedido.ToString("dd/MM/yyyy HH:mm"), item.colaborador, item.departamento, item.produto.ToUpper(),
