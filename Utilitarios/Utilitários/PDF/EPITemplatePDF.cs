...
namespace Utilitarios.Utilitários.PDF
{
    ...
    public class EPITemplatePDF
    {
        ...
        /// <returns></returns>
        public static string GetHTMLString(EPIDadosPDFDTO dadosPDF)
        {
            ...
            foreach (var item in dadosPDF.produtos)
            {
                sb.AppendFormat(@"<tr>
                                    ...
                                    <td>{5}</td>
                                    <td>{6}</td>
                                  </tr>", item.nomeProduto, item.tamanho, item.quantidade, item.dataVinculo.ToString("dd/MM/yyyy HH:mm"),
                                  item.dataDesvinculo.ToString("dd/MM/yyyy HH:mm") == "01/01/0001 00:00" ? "-" : item.dataDesvinculo.ToString("dd/MM/yyyy HH:mm"),
                                  item.validade.ToString("dd/MM/yyyy"), dadosPDF.nome);
            }
            sb.Append(@"
                                    </table>
                                </div>
